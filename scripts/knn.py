...
def knn(x, y, x_test, k=1, weights="uniform", recency=False, keep_na=False,
    ...
    When an annotator sees a tweet, it assigns to it the majority label of the kNN tweets labeled thus far.
    I suggest to set k=1 to begin with. This is not unnatural, I do not believe that anyone is able to remember e.g
    the k=10 most similar tweets.

    2. Alternative: Annotation by recency
    When an annotator sees a tweet, it assigns to it the label of the most recent similar tweet.
    The algo is:
    - sort all past tweets on similarity and rank them, smaller numbers for higher similarity, e.g. 3, 2, 5, 4, 1
    - rank all past tweets on recency, smaller numbers for higher recency, e.g. 1, 2, 3, 4, 5
    - choose the most similar tweet where the recency rank is not larger than the similarity rank, ie. 2 in the concrete case.
...
def _predict_label(ys, dists, weights):
    """
    Predicts the label of an unknown sentence given its neighbor labels, the
    neigbors' distances to that point and the weighting scheme how to aggregate
    ...
    -------
    str.
    Majority label for unknown sentence according to its k-nearest neighbors.
    ...
    if weights == "uniform":
        counts = Counter(ys)
        # Get only the tuple with most counts: (label, count)
        # print "counts", counts
...
def compute_micro_metrics(counts):
    ...
    # If there are no annotators, which could happen for some strata, the
    # divisor might be 0, so we need to deal with it
    final = sum(counts, Counter())
    if final["total_preds"] > 0:
        prec = 1.0*final["correct"] / final["total_preds"]
    if final["total_truths"] > 0:
...
def test_neighbor_uniform_weight():
    ...
    train = [s3, s1, s4, s2, s5]
    labels = ["a", "a", utility.NA, utility.NA, utility.NA]
    idxs, dists = _get_neighbors(train, s_test, k=3, recency=False,
                                 similarity="substring")
    neighbor_labels = []
    # Get labels of neighbors
    ...
    train = [s3, s1, s4, s2, s5]
    labels = ["a", "a", utility.NA, "a", utility.NA]
    idxs, dists = _get_neighbors(train, s_test, k=3, recency=False,
                                 similarity="substring")
    print "NA distances", idxs, dists
    neighbor_labels = []
...
def test_neighbor_distance_weight():
    ...
    y_t = _predict_label(neighbor_labels, dists, weights="distance")
    assert(y_t == "b")
...
def test_hierarchical_precision_recall_f1():
    ...
    truth = [[utility.NA]]
    predicted = [[utility.NA, utility.NA]]
    prec, rec, f1, _ = hierarchical_precision_recall_f1(predicted, truth)
    assert(prec == 0 and rec == 0 and f1 == 0)
...
def test_na():
    ...
    x, y = _remove_na_texts([s1, s2, s3, s4, s5], ["a", "b", "a", "b",
                                                   utility.NA])
    assert(x == [s1, s2, s3, s4] and utility.NA not in y)
